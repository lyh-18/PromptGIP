...
import models_mae, models_mae_lyh
...
def prepare_model(chkpt_dir, arch='mae_vit_large_patch16', img_size=224, device='cpu', strict=True):
    # build model
...
def prepare_model_lyh(chkpt_dir, arch='mae_vit_large_patch16', img_size=224, device='cpu', strict=True):
    # build model
    model = getattr(models_mae_lyh, arch)(img_size=img_size)
    # load model
    checkpoint = torch.load(chkpt_dir, map_location='cpu')
    msg = model.load_state_dict(checkpoint['model'], strict=strict)
    print(msg)
...
def generate_image_lyh(orig_image, model, ids_shuffle, num_patches, len_keep, device='cpu'):
    """ids_shuffle is [bs, 196]"""
    mask, orig_image, x = generate_raw_prediction(device, ids_shuffle, len_keep, model, orig_image)
    ...
    y = x.argmax(dim=-1)
    im_paste, mask, orig_image = decode_raw_predicion_lyh(mask, model, num_patches, orig_image, y)
    return orig_image, im_paste[0], mask
...
def decode_raw_predicion_lyh(mask, model, num_patches, orig_image, y):
    y = model.vae.quantize.get_codebook_entry(y.reshape(-1),
                                              [y.shape[0], y.shape[-1] // num_patches, y.shape[-1] // num_patches, -1])
    y = model.vae.decode(y)
    # plt.figure(); plt.imshow(y[0].permute(1,2,0)); plt.show()
    y = F.interpolate(y, size=(orig_image.shape[2], orig_image.shape[3]), mode='bilinear').permute(0, 2, 3, 1)
    y = torch.clip(y * 255, 0, 255).int().detach().cpu()
    # visualize the mask
    mask = mask.unsqueeze(-1).repeat(1, 1, model.patch_embed.patch_size[0] ** 2 * 3)
    mask = model.unpatchify(mask)  # 1 is removing, 0 is keeping
...
def generate_raw_prediction(device, ids_shuffle, len_keep, model, orig_image):
    ...
    # add pos embed
    #print('x: ', x.shape)
    #print('decoder_pos_embed: ', model.decoder_pos_embed.shape)
